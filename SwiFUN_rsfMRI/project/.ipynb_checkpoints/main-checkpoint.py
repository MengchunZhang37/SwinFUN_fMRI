...
def cli_main():
    ...
    parser.add_argument("--swift_patch_size", type=int, nargs=4, default=[2, 2, 2, 2],
                    help="Patch size for each of [H, W, D, T]")
    parser.add_argument("--swift_window_size", type=int, nargs=4, default=[2, 2, 2, 2],
                    help="Window size for each of [H, W, D, T]")
    ...
    if args.loggername == "tensorboard":
        ...
        # Allow --project_name to be written as "entity/project" or just "project"
        entity, project = (None, args.project_name)
        if "/" in args.project_name:
            entity, project = args.project_name.split("/", 1)
            
        run_id = args.run_id if args.run_id is not None else exp_id

        # If you want to “resume training from a checkpoint using the same W&B run,” 
        # you can pass exp_id to id (optional)
        logger = WandbLogger(
            project=project,
            entity=entity,
            save_dir=args.default_root_dir,
            log_model=False,   # Let ModelCheckpoint handle local weights; set True if you need W&B artifacts
            id=run_id,         # None if not provided
            resume="allow"
        )
        # Use the W&B run id as args.id to ensure predictions/{id} stays consistent
        if run_id is None:
            setattr(args, "id", logger.version)  # logger.version == wandb.run.id
        dirpath = os.path.join(args.default_root_dir, str(logger.version))
    ...
    # callback for regression task
        ...
        checkpoint_callback = ModelCheckpoint(
            dirpath=dirpath,
            monitor="valid_mse",
            filename="ep{epoch:02d}-mse{valid_mse:.4f}",
            mode="min",
            save_top_k=1,    # only keep the best
            save_last=True,  # and keep the last
        )
