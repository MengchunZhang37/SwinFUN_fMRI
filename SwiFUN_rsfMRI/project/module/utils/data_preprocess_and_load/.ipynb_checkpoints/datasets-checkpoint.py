...
class BaseDataset(Dataset):
    def __init__(self, **kwargs):
        ...
        self._printed_none_warning = False
    ...
    def load_sequence(self, subject_path, start_frame, sample_duration, num_frames=None): 
        ...
        if self.input_scaling_method == 'none':
            if not self._printed_none_warning:
                print('[INFO] Assume that normalization already done (preprocessing v1)')
                self._printed_none_warning = True
            pass
        else:
...
class DS000030(BaseDataset):
    ...
    def __getitem__(self, index):
        ...
        # === pad to (1,96,96,96,T) ===
        H, W, D = y.shape[1], y.shape[2], y.shape[3]  # y input (1, H, W, D, T)
        if (H, W, D) != (96, 96, 96):
            # Choose a stable background fill value (consistent with preprocessing background; 0 is also fine).
            bg = float(y.flatten()[0]) if y.numel() > 0 else 0.0

            # First convert y to (N=1, C=T, D, H, W) to match 3D pad's dimension order (W, H, D).
            y_perm = y.permute(0, 4, 3, 1, 2)   # (1, T, D, H, W)
            ...
            # Order for F.pad: (W_left, W_right, H_left, H_right, D_left, D_right)
            y_perm = F.pad(y_perm, (w0, w1, h0, h1, d0, d1), mode="constant", value=bg)

            # Convert back to (1, H, W, D, T)
            y = y_perm.permute(0, 3, 4, 2, 1)
        ...
        elif self.downstream_task == 'rfMRI_next':
