...
from .data_preprocess_and_load.datasets import S1200, ABCD, UKB, Dummy, DS000030
...
class fMRIDataModule(pl.LightningDataModule):
    def __init__(self, **kwargs):
        ...
        self.save_hyperparameters(ignore=["id"])
...
#         self.split_file_path = os.path.join(split_dir_path, f"split_fixed_{self.hparams.dataset_split_num}.txt")
        ...
        self.split_file_path = getattr(self.hparams, "split_file_path", None)           
        self.save_random_split_to = getattr(self.hparams, "save_random_split_to", None)  
    ...
    def get_dataset(self):
        if self.hparams.dataset_name == "Dummy":
            ...
            return UKB
        elif self.hparams.dataset_name == "DS000030": 
            return DS000030
        else:
    ...
    def save_split(self, sets_dict):
        if self.split_file_path is None:
            return
    ...
    def save_split_to(self, path, sets_dict):
    ...
    def determine_split_randomly(self, S):
        ...
        np.random.seed(self.hparams.dataset_split_num)
        ...
        S_train = subjects[:n_train]
        S_val = subjects[n_train:n_train + n_val]
        S_test = subjects[n_train + n_val:]

        if self.save_random_split_to is not None:
            self.save_split_to(self.save_random_split_to, {
                "train_subjects": S_train,
                "val_subjects": S_val,
                "test_subjects": S_test
            })
            print(f"[INFO] Random split saved to: {self.save_random_split_to}")


        print(f"determine_split_randomly returning: train={len(S_train)}, val={len(S_val)}, test={len(S_test)}")

        return S_train, S_val, S_test
    
    def load_split(self):
        if self.split_file_path is None or not os.path.isfile(self.split_file_path):
            raise FileNotFoundError(f"split_file_path must be an existing file, got: {self.split_file_path}")
        with open(self.split_file_path, "r") as f:
            subject_order = [line.rstrip("\n") for line in f]
    ...
    def make_subject_dict(self):
        ...
        if self.hparams.dataset_name == "S1200":
            ...
            meta_data_path = os.path.join(self.hparams.image_path, "metadata", "your_task.csv")
            if not os.path.exists(meta_data_path):
                raise FileNotFoundError(f"Metadata file not found at {meta_data_path}")
            meta_data = pd.read_csv(meta_data_path)

            # Check available subjects in img directory
            img_root = os.path.join(self.hparams.image_path, 'img')
            available_subjects = set(os.listdir(img_root)) if os.path.exists(img_root) else set()
            print(f"Found {len(available_subjects)} subjects in img directory")
            print(f"Metadata contains {len(meta_data)} entries")
            ...
            for _, row in meta_data.iterrows():  
                ...
                if subject not in available_subjects:
                    print(f"Warning: Subject {subject} not found in img directory, skipping...")
                    continue

                # Get sex and target path from metadata
                sex = row.get('sex', 0)

                if self.hparams.downstream_task == 'tfMRI_3D':
                    target_path = row['label_path']

                    # Convert relative path to absolute path if needed
                    if not os.path.isabs(target_path):
                        # If it's a relative path starting with './', remove the prefix
                        if target_path.startswith('./'):
                            target_path = os.path.join(os.getcwd(), target_path[2:])
                        else:
                            target_path = os.path.join(os.getcwd(), target_path)

                    # Verify target file exists
                    if not os.path.exists(target_path):
                        print(f"Warning: Target file {target_path} not found for subject {subject}, skipping...")
                        continue
                    ...
                    final_dict[subject] = [sex, target_path]
                    valid_count += 1
            print(f"Successfully loaded {valid_count} valid subject-label pairs")

            if valid_count == 0:
                print("ERROR: No valid subjects found!")
                print("Debug info:")
                print(f"  - Image path: {self.hparams.image_path}")
                print(f"  - Img root: {img_root}")
                print(f"  - Metadata path: {meta_data_path}")
                print(f"  - Current working directory: {os.getcwd()}")
                print(f"  - Available subjects sample: {list(available_subjects)[:5] if available_subjects else 'None'}")
                if len(meta_data) > 0:
                    print(f"  - Metadata subjects sample: {meta_data['Subject'].tolist()[:5]}")
                    print(f"  - Sample label path: {meta_data['label_path'].iloc[0] if len(meta_data) > 0 else 'None'}")
        
        print(f"[DEBUG in make_subject_dict] final_dict has {len(final_dict)} entries")
    ...
    def setup(self, stage=None):
        ...
        use_saved = (self.split_file_path is not None) and os.path.isfile(self.split_file_path)
        if use_saved:
    ...
    def add_data_specific_args(cls, parent_parser: ArgumentParser, **kwargs) -> ArgumentParser:
        ...
        group.add_argument("--stride_within_seq", type=float, default=1, help="ssubject_dict = self.make_subject_dict()kip some fMRI volumes within fMRI sub-sequences")
        ...
        group.add_argument("--save_random_split_to", type=str, default=None,help="Optional file path to SAVE the random split. Not used for loading.")
        group.add_argument("--split_file_path", type=str, default=None,help="Optional file path to LOAD a previously saved split. If not set, nothing is loaded.")
