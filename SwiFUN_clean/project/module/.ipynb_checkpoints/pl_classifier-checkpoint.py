...
class LitClassifier(pl.LightningModule):
    def __init__(self, data_module, **kwargs):
        ...
        self.run_id = kwargs.get("id", "default_id")
        ...
        if self.hparams.downstream_task == 'tfMRI_3D' and self.hparams.mask_input:
            mask_path = os.path.join(self.hparams.image_path, 'ROI', self.hparams.mask_filename)
            if not os.path.exists(mask_path):
                raise FileNotFoundError(f"Mask input is enabled, but mask file not found: {mask_path}")

            #mask = torch.tensor(nb.load(mask_path).get_fdata()) 
            ...
            with torch.no_grad():
                # [1,1,96,96,96] -> interpolate -> [1,1,6,6,6] -> [6,6,6]bool
                _m = self.MNI152_mask[None, None, ...].float()
                _m_ds = F.interpolate(_m, size=(6, 6, 6), mode='nearest')[0, 0].to(torch.bool)
            # Save as buffer，, so it can be reused directly during training/validation (will be automatically moved with model.to(device)).
            self.register_buffer('MNI152_mask_ds', _m_ds)                  # [6,6,6] bool
            self.register_buffer('MNI152_mask_ds_flat', _m_ds.reshape(-1)) # [216]  bool
